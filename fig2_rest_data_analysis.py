...
path = '' # path to data with flies with a rest flight trials
...
for experiment_name in experiment_names:
    ...
    for sun_period in sun_periods:
        ...
        heading_angle = angle_avg(delayed_sun_period['Heading Angle']) 
        heading_angle_var = angle_var(delayed_sun_period['Heading Angle'])
...
for x in range (allFliesAngleVarsA.size):
    if allFliesAngleVarsA[x] > 0.8 or allFliesAngleVarsB[x] > 0.8:                                     
        pass
    else:
...
LinearPlot(rotated_heading_anglesA, rotated_heading_anglesB, allFliesAngleVarsACutOff,allFliesAngleVarsACutOff,x_axis = 'Sun1 Headings', y_axis = 'Sun2 Headings', title = 'Sun1-Sun2 Headings')
...
BootstrapAnalysis(rotated_heading_anglesA, rotated_heading_anglesB, 'Sun1-Sun2 Bootstrap Results')
...
PlotSeaUrchin(rotated_heading_anglesA, allFliesVecStrengthCutOffA, 'Sun1 heading polar plot')
PlotSeaUrchin(rotated_heading_anglesB, allFliesVecStrengthCutOffB, 'Sun2 heading polar plot')
...
sun_position_change = difference(allFliesSunACutOff, allFliesSunBCutOff, deg = True)
# Replace these lines with your actual data
heading_difference = heading_difference
sun_position_change = sun_position_change
motor_state = np.full(len(heading_difference), 'rest')
visual_stimuli = np.full(len(heading_difference), 'sun')
# Create a DataFrame
df = pd.DataFrame({
    'heading_difference': heading_difference,
    'sun_position_change': sun_position_change,
