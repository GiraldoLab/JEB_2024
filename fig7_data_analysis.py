#!/usr/bin/env python
#Plots polar and linear plots for plotting flies headings for fig7
#Plots heading difference between UV sun1 & UV sun2, UV sun2 & UV sun3 as a polar plot and resampling distribution (bootstrap analysis)
...
FirstSunFliesSunCutOff = np.zeros (1)

SecondSunFliesAngleVarsCutOff = np.zeros (1)
SecondSunFliesMeanAnglesCutOff = np.zeros (1)
SecondSunFliesSunCutOff = np.zeros (1)

ThirdSunFliesAngleVarsCutOff = np.zeros (1)
ThirdSunFliesMeanAnglesCutOff = np.zeros (1)
ThirdSunFliesSunCutOff = np.zeros (1)
...
for x in range (first_heading_vars.size):
    if first_heading_vars[x] > 0.8 or second_heading_vars[x] > 0.8 or third_heading_vars[x]>0.8:                                      
        ...
        FirstSunFliesSunCutOff  = np.append(FirstSunFliesSunCutOff , first_sun_positions[x])
        ...
        SecondSunFliesSunCutOff = np.append(SecondSunFliesSunCutOff, second_sun_positions[x])

        ThirdSunFliesAngleVarsCutOff= np.append(ThirdSunFliesAngleVarsCutOff, third_heading_vars[x])
        ThirdSunFliesMeanAnglesCutOff = np.append(ThirdSunFliesMeanAnglesCutOff, third_heading_angles[x])
        ThirdSunFliesSunCutOff = np.append(ThirdSunFliesSunCutOff, third_sun_positions[x])

        
FirstSunFliesAngleVarsCutOff = FirstSunFliesAngleVarsCutOff[1:]
FirstSunFliesMeanAnglesCutOff =FirstSunFliesMeanAnglesCutOff[1:]
FirstSunFliesSunCutOff = FirstSunFliesSunCutOff[1:]
...
SecondSunFliesSunCutOff = SecondSunFliesSunCutOff[1:]

ThirdSunFliesAngleVarsCutOff= ThirdSunFliesAngleVarsCutOff[1:]
ThirdSunFliesMeanAnglesCutOff = ThirdSunFliesMeanAnglesCutOff[1:]
ThirdSunFliesSunCutOff = ThirdSunFliesSunCutOff[1:]
...
ThirdSunFliesVecStrengthCutOff = 1- ThirdSunFliesAngleVarsCutOff

rotated_heading_anglesA=difference(FirstSunFliesMeanAnglesCutOff, FirstSunFliesSunCutOff, deg=True)
rotated_heading_anglesB=difference(SecondSunFliesMeanAnglesCutOff, SecondSunFliesSunCutOff, deg=True)
rotated_heading_anglesC=difference(ThirdSunFliesMeanAnglesCutOff, ThirdSunFliesSunCutOff, deg=True)

heading_difference1 = difference(rotated_heading_anglesA, rotated_heading_anglesB, deg=True)
heading_difference2 = difference(rotated_heading_anglesB, rotated_heading_anglesC, deg=True)
...
LinearPlot(rotated_heading_anglesA, rotated_heading_anglesB,FirstSunFliesAngleVarsCutOff,SecondSunFliesAngleVarsCutOff,x_axis =' First Sun Headings', y_axis = 'Second Sun Headings', title = 'First_Second Sun Linearplot')
LinearPlot(rotated_heading_anglesB, rotated_heading_anglesC,SecondSunFliesAngleVarsCutOff,ThirdSunFliesAngleVarsCutOff,x_axis =' Second Sun Headings', y_axis = 'Third Sun Headings', title = 'Second_third Sun Linearplot')
...
BootstrapAnalysis(rotated_heading_anglesB, rotated_heading_anglesC, 'Second-Third Sun Bootstrap Results')
...
PlotSeaUrchin(rotated_heading_anglesA,FirstSunFliesVecStrengthCutOff, 'First Sun Polar plot')
PlotSeaUrchin(rotated_heading_anglesB, SecondSunFliesVecStrengthCutOff, 'Second Sun Polar plot')
PlotSeaUrchin(rotated_heading_anglesC,ThirdSunFliesVecStrengthCutOff, 'Third Sun Polar plot')
PlotSeaUrchin(heading_difference1, np.ones_like(heading_difference1), 'first_second Heading Difference polarplot')
PlotSeaUrchin(heading_difference2, np.ones_like(heading_difference2), 'second_third Difference polarplot')
